...
test('v1.0.0 exposes private subscribed calendar UI and client',()=>{
  ...
  assert.match(html,/calendar-subscriptions\.js\?v=1\.0\.0/);
...
test('worker stores calendar feeds in a Durable Object',()=>{
  ...
  assert.match(worker,/export class CalendarStore/);
  ...
  assert.equal(cfg.migrations[0].new_sqlite_classes[0],'CalendarStore');
});
