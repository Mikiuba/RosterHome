...
for(const file of fs.readdirSync(out)){
  if(!allowed.has(file))throw Error('Unexpected existing build file: '+file);
}
...
const parts=fs.readdirSync(partsDir).filter(f=>/^[0-9]+\.txt$/.test(f)).sort();
if(!parts.length)throw Error('No enhancement fragments found.');
const combined=parts.map(f=>fs.readFileSync(path.join(partsDir,f),'utf8')).join('');
fs.writeFileSync(path.join(out,'enhancements.js'),combined,'utf8');

console.log('Built '+(files.length+1)+' public assets; enhancements.js from '+parts.length+' small fragments.');
